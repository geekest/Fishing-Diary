...
- PR #11：`fix/share-preview-scale`，现有分享卡模板与预览导出链路。已确认可干净合并并完成模拟器 Debug 构建。
- PR #12：`docs/no-paid-roadmap`，明确无付费开发者账号下的本地优先路线。
- PR #13：`feat/local-log-export`，在“我的”页新增本地 CSV 导出。
- PR #14：`feat/manual-env-fallback`，补充环境数据本地预设和手动兜底提示。
- PR #15：`feat/sample-diary-data`，在日记空态新增演示记录入口。

验证结果：

- #11、#13、#14、#15 均已运行 iOS Simulator Debug build，结果通过。
- #12 为文档与计划变更，已运行 `git diff --check`。

与原计划不同：

- #11 清理冲突残留后与当前 HEAD 一致，因此没有追加 commit。
- 为避免工程文件噪音，#13 的 CSV 导出逻辑集中在现有 `ProfileView.swift`，未新增 Swift 文件。

未验证：

- 未启动模拟器逐页截图或手动点击新入口。
- 未验证 PR 合并后的组合效果；所有 PR 均以 `main` 为 base，需要按依赖关系或冲突情况逐个合并。
